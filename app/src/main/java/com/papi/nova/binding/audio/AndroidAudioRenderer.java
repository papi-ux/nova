...
public class AndroidAudioRenderer implements AudioRenderer {
    ...
    private volatile boolean trackStarted;
    ...
    public int setup(MoonBridge.AudioConfiguration audioConfiguration, int sampleRate, int samplesPerFrame) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            try {
                ...
                // Don't play() yet — defer until first audio data arrives in playDecodedAudio()
                // to avoid buffer underrun static during the RTP sync period.
                trackStarted = false;
    ...
    // Static reference for audio haptic engine — set by Game.java
    public static volatile com.papi.nova.ui.AudioHapticEngine hapticEngine;

    @Override
    public void playDecodedAudio(short[] audioData) {
        // Start AudioTrack on first real audio data — avoids buffer underrun static
        if (!trackStarted) {
            track.play();
            trackStarted = true;
        }

        // Feed audio haptic engine (bass → vibration)
        if (hapticEngine != null) {
            hapticEngine.feedAudioShort(audioData, track.getSampleRate(), track.getChannelCount());
        }
    ...
    @Override
    public void cleanup() {
        // Only pause/flush if the track was actually started
        if (trackStarted) {
            track.pause();
            track.flush();
        }
