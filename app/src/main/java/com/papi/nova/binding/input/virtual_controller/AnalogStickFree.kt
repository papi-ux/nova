...
open class AnalogStickFree(controller: VirtualController, context: Context, elementId: Int) :
    ...
    private var bgCircleColor = 0x2BF5F5F9
    private var strokeCircleColor = 0xFF8F8F8F.toInt()
    ...
    fun setBgOpacity() {
        val hexOpacity = PreferenceConfiguration.readPreferences(context).enableNewAnalogStickOpacity * 255 / 100
        bgCircleColor = (hexOpacity shl 24) or (bgCircleColor and 0x00FFFFFF)
        strokeCircleColor = (hexOpacity shl 24) or (pressedColor and 0x00FFFFFF)
        invalidate()
    }
