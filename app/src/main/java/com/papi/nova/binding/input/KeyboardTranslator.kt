...
class KeyboardTranslator(private val prefConfig: PreferenceConfiguration) : InputManager.InputDeviceListener {
    private class KeyboardMapping @TargetApi(33) constructor(private val device: InputDevice) {
        private val deviceKeyCodeToQwertyKeyCode = IntArray(KeyEvent.getMaxKeyCode() + 1) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            for (deviceId in InputDevice.getDeviceIds()) {
                ...
                if (device != null && device.keyboardType == InputDevice.KEYBOARD_TYPE_ALPHABETIC) {
                    keyboardMappings.put(deviceId, KeyboardMapping(device))
                }
            }
        }
    }
    ...
    fun translate(keycode: Int, scancode: Int, deviceId: Int): Short {
        var adjustedKeycode = keycode

        if (prefConfig.forceQwerty && deviceId >= 0) {
            val mapping = keyboardMappings.get(deviceId)
            if (mapping != null) {
                val qwertyKeyCode = mapping.getQwertyKeyCodeForDeviceKeyCode(adjustedKeycode)
                if (qwertyKeyCode != KeyEvent.KEYCODE_UNKNOWN) {
                    adjustedKeycode = qwertyKeyCode
                }
            }
        }

        var translated = when {
            adjustedKeycode in KeyEvent.KEYCODE_0..KeyEvent.KEYCODE_9 ->
                (adjustedKeycode - KeyEvent.KEYCODE_0) + VK_0
            adjustedKeycode in KeyEvent.KEYCODE_A..KeyEvent.KEYCODE_Z ->
                (adjustedKeycode - KeyEvent.KEYCODE_A) + VK_A
            adjustedKeycode in KeyEvent.KEYCODE_NUMPAD_0..KeyEvent.KEYCODE_NUMPAD_9 ->
                (adjustedKeycode - KeyEvent.KEYCODE_NUMPAD_0) + VK_NUMPAD0
            adjustedKeycode in KeyEvent.KEYCODE_F1..KeyEvent.KEYCODE_F12 ->
                (adjustedKeycode - KeyEvent.KEYCODE_F1) + VK_F1
            else -> when (adjustedKeycode) {
        ...
        if (translated == 0) {
            if (hasNormalizedMapping(adjustedKeycode, deviceId)) {
                return 0
            }
        ...
        return ((KEY_PREFIX shl 8) or translated).toShort()
    }
    ...
    companion object {
        ...
        fun getModifier(key: Short): Byte {
            return when (key.toInt()) {
                VK_LSHIFT -> KeyboardPacket.MODIFIER_SHIFT
                VK_LCONTROL -> KeyboardPacket.MODIFIER_CTRL
                VK_LWIN -> KeyboardPacket.MODIFIER_META
                VK_LMENU -> KeyboardPacket.MODIFIER_ALT
                else -> 0
            }
        }
    }
