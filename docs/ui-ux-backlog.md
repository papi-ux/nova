...
| Medium | Stream startup capture | First screenshot after launch can be black even though logs show stream active and packets arriving. | If users report black-screen starts, collect a timed screenshot/video trace and correlate with decoder output/first rendered frame timing. |
...
| High | Stream startup visual feedback | Retroid smoke saw a black first capture while logs already showed stream active and packets arriving. Even if transient, users read black frames as failure. | Add or refine a lightweight startup state that distinguishes `connecting`, `stream active / waiting for first frame`, and `input ready`; collect timed screenshots/log markers before changing decoder code. |
