...
- 2026-05-23 populated Pixel phone smoke report: `/Users/papi/claude-hub/artifacts/nova/pixel-phone/two_zone_20260523_174704/manual/manual_summary.json`
  ...
  - Caveat: before the helper-oracle hardening in this pass, `tools/nova_retroid_smoke.py phone` still carried old dashboard/base-library label expectations and reported a false `FAIL`; the current helper now targets the paired two-zone Library base, Library Options drawer, System drawer, and Back dismissal path. A fresh post-recovery Pixel device run is still pending when Pixel wireless ADB is available.
...
| Phone helper oracle hardening | Passed | `python3 -m unittest tools.test_nova_retroid_smoke` passed after updating the `phone` smoke helper to accept paired-launch-to-Library, reject drawer-owned filters on the base phone Library, capture/analyze Library Options and System drawers, and verify Back returns to the Library shell. Fresh post-recovery Pixel hardware smoke remains pending because Pixel wireless ADB was not advertised by `adb mdns services`. |
