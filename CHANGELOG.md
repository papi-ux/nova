...
Play Setup keeps host-wide choices such as Headless Dongle visible without presenting them as selectable per-game overrides. A stale per-game Headless Dongle choice from an older Nova is retired on upgrade. Pressing a host-only card opens the matching Polaris settings surface, while launch-mode and capture-path descriptions lead with display, privacy, and performance impact before backend terminology; Headless Dongle blanks the real panel only when the host uses Privacy mode, while Off mode leaves it active.
